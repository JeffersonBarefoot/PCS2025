...
return redirect('/incumbents')->with('success', 'Incumbent saved!');
  ...
  // dd($user->currentTeam->id);
    ...
    $reqcompany =             $incumbent->company;
    $reqempno =               $incumbent->empno;
    $reqid =                  $incumbent->id;
    $empno =                  $reqempno;
    $reqpositioncompany =     $incumbent->poscompany;
    // $reqpositionposno = $request->input('reqpositionposno');
    $reqpositionposno =       $incumbent->posno;
    $reqincumbenthistoryid =  $request->reqincumbenthistoryid;



  //  JEFF REMD THIS OUT 20230720 not sure where to get it  $reqincumbenthistoryid =  $request->input('reqincumbenthistoryid');
  // this should come through with the request string JLB 20230721
     if(empty($reqincumbenthistoryid)) {
       $reqincumbenthistoryid=1;
     }

    // returns the name of the selected position, to show at the top of the page
    ...
    //$incumbentxx = Incumbent::where('empno', '=' , $empno)
...
//    });
    ...
    $incumbentsnavbar = Incumbent::where('company','like',"$navbarcompany%")
      ...
      ->select('id','company','empno','fname','lname','active')
      ->distinct()
      ->orderby("company")
      ->orderby("lname")
      ->get();
  //  $dumprequest = $incumbentsnavbar;
...
// }
    ...
    return View('incumbents.show')
      ...
      ->with('empno',$empno)
      ->with('incumbentrequest',$request)
      ->with('dumprequest2',$dumprequest2);
